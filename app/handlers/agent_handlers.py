...
router.message.filter(
    F.chat.id == settings.SUPERGROUP_ID,
    F.message_thread_id.is_not(None)
)

@router.message(Command("close_chat"))
async def handle_close_chat_command(message: Message, bot: Bot):
    """
    ...
    with next(get_session()) as session:
        statement = select(SupportSession).where(
            SupportSession.topic_id == topic_id,
        ...
        if active_session.agent_telegram_id != agent_id:
            await message.reply("⛔️ Вы не можете закрыть эту сессию, так как она назначена на другого агента.")
            return
        ...
        if success:
            ...
            await message.reply("🔴 Произошла ошибка при закрытии сессии. Попробуйте снова.")
...
async def handle_agent_message(message: Message, bot: Bot):
    ...
    with next(get_session()) as session:
        ...
        if not active_session:
            logging.warning(f"Received message in topic {topic_id}, but no active session found.")
            return
